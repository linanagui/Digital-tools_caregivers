...
#convert vaues to meaningful labels
mutate(
...
Merge by Chronic Conditions:
	•	NHATS Chronic Condition Variables (has_heart_disease, has_hypertension, has_lung_disease, etc.)
	•	HRRP Measure Name (measure_name: “READM-30-HF-HRRP”, “READM-30-AMI-HRRP”, etc.)
...
# Create a mapping of NHATS conditions to HRRP measure names
condition_map <- tibble(
  nhats_condition = c("has_heart_disease", "has_lung_disease", "has_diabetes",
                      "has_cancer", "has_stroke", "has_arthritis"),
  hrrp_measure = c("READM-30-HF-HRRP", "READM-30-COPD-HRRP", "READM-30-PN-HRRP",
                   "READM-30-AMI-HRRP", "READM-30-CABG-HRRP", "READM-30-THA-TKA-HRRP")
)
```


```{r}
# # Reshape NHATS data to long format for merging
# nhats_long <- nhats_cleaned %>%
#   pivot_longer(cols = starts_with("has_"),
#                names_to = "nhats_condition",
#                values_to = "has_condition") %>%
#   filter(has_condition == 1) %>% # Keep only rows where the person has the condition
#   left_join(condition_map, by = "nhats_condition")  # Map to HRRP condition names
```
...
```{r}
# Merge with HRRP based on condition
merged_data <- nhats_long %>%
  left_join(hrrp_filtered, by = c("hrrp_measure" = "measure_name"))
```
